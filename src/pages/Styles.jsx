...
const Styles = () => {
    ...
    const [styleModal, setStyleModal] = useState({ open: false, data: null, categoryId: null });
    const [cameraModal, setCameraModal] = useState({ open: false, styleId: null });
    ...
    const fetchCategories = async () => {
        setIsLoading(true);
        try {
            const res = await axios.get('/api/styles/categories');
    ...
    const handleSaveCategory = async (e) => {
        ...
        try {
            ...
            const url = isEdit ? `/api/styles/categories/${categoryModal.data.id}` : '/api/styles/categories/add';
            const method = isEdit ? 'put' : 'post';
            await axios[method](url, { name: formName });
            setCategoryModal({ open: false, data: null });
            fetchCategories();
        } catch (err) {
            alert(t('styles.errorSavingCategory'));
        }
    };

    const confirmDeleteCategory = async () => {
        try {
            await axios.delete(`/api/styles/categories/${deleteData.id}`);
            setDeleteData({ open: false, type: null, id: null });
            fetchCategories();
        } catch (err) {
            alert(t('styles.errorDeletingCategory'));
        }
    };
    ...
    const handleSaveStyle = async (e) => {
        e.preventDefault();
        try {
            const isEdit = !!styleModal.data;
            const url = isEdit ? `/api/styles/${styleModal.data.id}` : '/api/styles/add';
            const method = isEdit ? 'put' : 'post';
            await axios[method](url, { name: formName, styleCategoryId: styleModal.categoryId });
            setStyleModal({ open: false, data: null, categoryId: null });
            fetchCategories();
        } catch (err) {
            alert(t('styles.errorSavingStyle'));
        }
    };

    const confirmDeleteStyle = async () => {
        try {
            await axios.delete(`/api/styles/${deleteData.id}`);
            setDeleteData({ open: false, type: null, id: null });
            fetchCategories();
        } catch (err) {
            alert(t('styles.errorDeletingStyle'));
        }
    };
    ...
    const handleFileUpload = async (e, styleId) => {
        const file = e.target.files[0];
        if (!file) return;
        await uploadImage(file, styleId);
    };
    ...
    const confirmCapture = async () => {
        ...
        await uploadImage(file, cameraModal.styleId);
        setCameraModal({ open: false, styleId: null });
        setCapturedImage(null);
    };

    const uploadImage = async (file, styleId) => {
        const formData = new FormData();
        formData.append('image', file);
        formData.append('styleId', styleId);
        try {
            await axios.post('/api/styles/images', formData);
            fetchCategories();
        } catch (err) {
            alert(err.response?.data?.message || t('styles.errorUploadingImage'));
        }
    };

    const handleDeleteImage = async (id) => {
        if (window.confirm(t('styles.deleteImageConfirm'))) {
            try {
                await axios.delete(`/api/styles/images/${id}`);
                fetchCategories();
            } catch (err) {
                alert(t('styles.errorDeletingImage'));
            }
        }
    };
    ...
    return (
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white dark:bg-gray-800 p-8 rounded-3xl shadow-sm border border-gray-100 dark:border-gray-700">
                <div>
                    <h1 className="text-4xl font-extrabold text-gray-900 dark:text-white tracking-tight">{t('styles.inventory')}</h1>
                    <p className="text-gray-500 dark:text-gray-400 mt-2 text-lg font-medium">{t('styles.subtitle')}</p>
                </div>
                <button
            ...
            {categories.length === 0 ? (
                <div className="bg-gray-50 dark:bg-gray-800/50 rounded-3xl p-12 text-center border-2 border-dashed border-gray-200 dark:border-gray-700">
                    ...
                    <p className="text-gray-500 mt-1">{t('styles.noCategoriesDesc')}</p>
                </div>
            ) : (
                            ...
                            >
                                <div className="flex items-center gap-4">
                                    ...
                                    <span className="text-xs font-semibold px-2.5 py-1 bg-gray-100 dark:bg-gray-700 text-gray-500 rounded-full">
                                        {t('styles.stylesCount', { count: category.Styles?.length || 0 })}
                                    </span>
                                </div>
                                <div className="flex items-center gap-3" onClick={e => e.stopPropagation()}>
                                    <button
                                        onClick={() => { setFormName(''); setStyleModal({ open: true, data: null, categoryId: category.id }); }}
                                        className="p-2 text-indigo-600 hover:bg-indigo-100 dark:hover:bg-indigo-900/40 rounded-lg transition"
                                        title={t('styles.addStyle')}
                                    >
                                        <Plus size={18} />
                                    </button>
                                    <button
                            ...
                            {expandedCategories[category.id] && (
                                <div className="divide-y divide-gray-50 dark:divide-gray-700">
                                    {(!category.Styles || category.Styles.length === 0) ? (
                                        <div className="p-8 text-center text-gray-400 text-sm">{t('styles.noStyles')}</div>
                                    ) : (
                                        category.Styles.map(style => (
                                            <div key={style.id} className="p-6 hover:bg-gray-50/50 dark:hover:bg-gray-900/20 transition-colors">
                                                <div className="flex items-center justify-between mb-4">
                                                    <h4 className="font-bold text-gray-800 dark:text-gray-200">{style.name}</h4>
                                                    <div className="flex items-center gap-2">
                                                        <button
                                                            onClick={() => { setFormName(style.name); setStyleModal({ open: true, data: style, categoryId: category.id }); }}
                                                            className="p-2 text-gray-600 hover:bg-gray-100 dark:hover:bg-gray-700/50 rounded-xl transition-colors"
                                                            title={t('styles.editStyle')}
                                                        >
                                                            <Edit2 size={18} />
                                                        </button>
                                                        <button
                                                            onClick={() => setDeleteData({ open: true, type: 'style', id: style.id })}
                                                            className="p-2 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition-colors"
                                                            title={t('styles.deleteStyle')}
                                                        >
                                                            <Trash2 size={18} />
                                                        </button>
                                                    </div>
                                                ...
                                                <div className="grid grid-cols-3 sm:grid-cols-5 md:grid-cols-9 gap-3">
                                                    {/* Add Button */}
                                                    {(style.StyleImages?.length || 0) < 9 && (
                                                        <button
                                                            onClick={() => setCameraModal({ open: true, styleId: style.id })}
                                                    ...
                                                    {style.StyleImages?.map(img => (
                                                        <div key={img.id} className="relative aspect-square rounded-xl overflow-hidden group border border-gray-100 dark:border-gray-700">
                                                            <img
                                                                src={`/api/styles/image/${img.filename}`}
                                                                alt={style.name}
                                                                className="w-full h-full object-cover transition-transform group-hover:scale-110 cursor-zoom-in"
                                                                onClick={() => setViewerModal({ open: true, url: `/api/styles/image/${img.filename}` })}
                                                            />
                                                            <button
                    ...
                    <div className="relative bg-white dark:bg-gray-800 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-zoom-in">
                        ...
                        <div className="p-8">
                            <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">
                                {categoryModal.data ? t('styles.editCategoryModal') : t('styles.newCategoryModal')}
                            </h3>
                            <form onSubmit={handleSaveCategory} className="space-y-6">
                                <div>
                                    <label className="block text-sm font-bold text-gray-500 mb-2 uppercase tracking-widest">{t('styles.nameLabel')}</label>
                                    <input
                                        autoFocus
                                        required
                                ...
                                <div className="flex gap-4">
                                    <button type="button" onClick={() => setCategoryModal({ open: false, data: null })} className="flex-1 p-4 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-500 font-bold hover:bg-gray-50 transition">{t('styles.cancel')}</button>
                                    <button type="submit" className="flex-1 p-4 rounded-xl bg-indigo-600 text-white font-bold hover:bg-indigo-700 transition shadow-lg shadow-indigo-500/20">{t('styles.save')}</button>
                                </div>
                            </form>
                        </div>
                    </div>
            ...
            {styleModal.open && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setStyleModal({ open: false, data: null, categoryId: null })} />
                    <div className="relative bg-white dark:bg-gray-800 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-zoom-in">
                        <button
                            onClick={() => setStyleModal({ open: false, data: null, categoryId: null })}
                            className="absolute top-4 right-4 p-2 text-gray-400 hover:text-gray-600 dark:hover:text-white rounded-full hover:bg-gray-100 dark:hover:bg-gray-700 z-10"
                        >
                            <X size={20} />
                        </button>
                        <div className="p-8">
                            <h3 className="text-2xl font-bold text-gray-900 dark:text-white mb-6">
                                {styleModal.data ? t('styles.editStyleModal') : t('styles.newStyleModal')}
                            </h3>
                            <form onSubmit={handleSaveStyle} className="space-y-6">
                                <div>
                                    <label className="block text-sm font-bold text-gray-500 mb-2 uppercase tracking-widest">{t('styles.styleNameLabel')}</label>
                                    <input
                                        autoFocus
                                        required
                                        className="w-full bg-gray-50 dark:bg-gray-700 border-none rounded-xl p-4 text-gray-900 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none"
                                        placeholder={t('styles.styleNamePlaceholder')}
                                        value={formName}
                                        onChange={e => setFormName(e.target.value)}
                                    />
                                </div>
                                <div className="flex gap-4">
                                    <button type="button" onClick={() => setStyleModal({ open: false, data: null, categoryId: null })} className="flex-1 p-4 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-500 font-bold hover:bg-gray-50 transition">{t('styles.cancel')}</button>
                                    <button type="submit" className="flex-1 p-4 rounded-xl bg-indigo-600 text-white font-bold hover:bg-indigo-700 transition shadow-lg shadow-indigo-500/20">{t('styles.save')}</button>
                                </div>
                            </form>
                        </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    ...
                    <div className="relative bg-white dark:bg-gray-800 rounded-3xl shadow-2xl w-full max-w-md overflow-hidden animate-bounce-in">
                        <div className="p-8 text-center">
                            ...
                            <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">{t('styles.deleteConfirmTitle')}</h3>
                            <p className="text-gray-500 dark:text-gray-400 mb-8 leading-relaxed">
                                {deleteData.type === 'category'
                                    ? t('styles.deleteCategoryWarning')
                                    : t('styles.deleteStyleWarning')}
                            </p>
                            <div className="flex gap-4">
                                <button onClick={() => setDeleteData({ open: false, id: null, type: null })} className="flex-1 p-4 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-500 font-bold hover:bg-gray-50 transition">{t('styles.cancel')}</button>
                                <button
                                    onClick={deleteData.type === 'category' ? confirmDeleteCategory : confirmDeleteStyle}
            ...
            {cameraModal.open && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
                    <div className="absolute inset-0 bg-black/90 backdrop-blur-md" onClick={() => setCameraModal({ open: false, styleId: null })} />
                    <div className="relative bg-gray-900 rounded-3xl shadow-2xl w-full max-w-lg overflow-hidden animate-slide-up h-[600px] flex flex-col">
                        <div className="p-4 border-b border-gray-800 flex justify-between items-center bg-black/20">
                            <h3 className="font-bold text-white">{t('styles.captureTitle')}</h3>
                            <button onClick={() => setCameraModal({ open: false, styleId: null })} className="text-gray-400 hover:text-white transition">
                                <X size={24} />
                            </button>
                        </div>
                        ...
                        <div className="p-8 bg-black/40 backdrop-blur-md">
                            {capturedImage ? (
                                <div className="flex gap-4">
                                    ...
                                    >
                                        <RefreshCw size={20} /> {t('styles.retake')}
                                    </button>
                                    <button
                                    ...
                                    >
                                        <Check size={20} /> {t('styles.usePhoto')}
                                    </button>
                                </div>
                            ) : (
                                <div className="flex items-center justify-between gap-6">
                                    <label className="p-4 bg-gray-800 text-white rounded-full hover:bg-gray-700 transition cursor-pointer">
                                        ...
                                        <input type="file" className="hidden" accept="image/*" onChange={(e) => {
                                            handleFileUpload(e, cameraModal.styleId);
                                            setCameraModal({ open: false, styleId: null });
                                        }} />
                                    </label>
